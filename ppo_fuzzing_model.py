...
class ActorNwk(nn.Module):
    ...
    def save_checkpoint(self):
        with open('actor_std_dev.txt', 'w') as f:
            f.write(str(self.action_std))
        torch.save(self.state_dict(), self.checkpoint_file)

    def load_checkpoint(self):
        with open('actor_std_dev.txt', 'r') as f:
            st_dev = float(f.readline().strip())
        self.action_std = st_dev
        self.action_var = torch.full((self.out_dim,), self.action_std * self.action_std).to(self.device)
...
class Agent():
    def __init__(self, gamma, policy_clip,lamda, adam_lr,
        ...
        self.critic = CriticNwk(num_chars=num_chars, char_embedding_size=char_embedding_size,adam_lr=adam_lr,chekpoint_file='tmp/critic')
